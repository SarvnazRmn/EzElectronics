...
const dbq_newProduct = "INSERT INTO products(model, category, quantity, details, sellingPrice, arrivalDate) VALUES VALUES ('IPhone55', 'Smartphone', 50, NULL, 8, '2024-05-15')"
...
describe("T1 - getCart | DAO", () => {
    ...
    test("T1.1.3 - two carts, one current : It should return the current user cart", async () => {
        db.run(dbq_userUnpaidEmpty1, [], async () => {
            db.run(dbq_userPaid1, [], async () => {
                db.get("SELECT * FROM carts WHERE customer = 'User1' AND paid = 1",
                [], 
                async (err: Error , cart: any) => { 
                    usercartId = cart.id;
                    db.run(dbq_newProduct, [], async () => {
                        db.run(dbq_add2ToCart, [usercartId], async () => {
                            const response = await cartDAO.getCart(testuser1);
                            expect(response).toEqual(userUnpaidEmpty1);
...
describe("T2 - addToCart | DAO", () => {
    ...
    test("T1.2.1 - empty db : It should create the current user cart, with new product", async () => {
        ...
        db.get("SELECT * FROM carts WHERE customer = 'User1' AND paid = 0", 
        [], 
        async (err: Error , cart: any) => { 
            usercartId = cart.id;
            db.all("SELECT * FROM cartItems WHERE cartId = ?", 
            [usercartId], 
    ...
    test("T1.2.4 - product out-of-stock : It should return a 409 error", async () => {
        db.run(dbq_newOosProduct, [], async () => {
        ...
        db.run(dbq_newProduct, [], async () => {
            db.run(dbq_userUnpaidEmpty1, [], async () => {
                ...
                async (err: Error , cart: any) => { 
                    ...
                    db.run(dbq_add2ToCart, [usercartId], async () => {
                        const response = await cartDAO.removeProductFromCart(testuser1, "IPhone55");
                        db.get("SELECT quantity_in_cart FROM cartItems WHERE cartId = ? AND product_model = IPhone55", 
                        [usercartId], 
                        async (err: Error , quantity: any) => {
...
describe("T6 - clearCart | DAO", () => {
    ...
    test("T1.6.2 - no errors case (empty cart) : should do nothing", async () => {
        db.run(dbq_userUnpaidEmpty1, [], async () => {
            ...
            async (err: Error , cart: any) => { 
                usercartId = cart.id; 
                db.all("SELECT quantity_in_cart FROM cartItems WHERE cartId = ?", 
                [usercartId], 
                async (err: Error , rows: any[]) => {
...
describe("T7 - deleteAllCarts | DAO", () => {
    test("T1.7.2 - multiple carts : should delete all carts in db", async () => {
        db.run(dbq_userUnpaidEmpty2, [], async () => {
            db.run(dbq_userPaid2, [], async () => {
                db.run(dbq_userPaid1, [], async () => {
                    db.run(dbq_userUnpaid1, [], async () => {
                        const response = await cartDAO.deleteAllCarts();
                        db.all("SELECT * FROM carts", 
                        [], 
                        async (err: Error , carts: any[]) => {  
