...
describe("ProductDAO", () => {
    ...
    describe("sellProduct", () => {
        ...
        test("should reject with EmptyProductStockError if the product stock is empty", async () => {
            const mockDBGet = jest.spyOn(db, "get").mockImplementation((sql, params, callback) => {
                callback(null, { arrivalDate: "2024-06-06", quantity: 0 });
                return {} as Database;
            });
        ...
        test("should reject with LowProductStockError if the requested quantity is greater than available stock", async () => {
            const mockDBGet = jest.spyOn(db, "get").mockImplementation((sql, params, callback) => {
                callback(null, { arrivalDate: "2024-06-06", quantity: 3 });
                return {} as Database;
            });
        ...
        test("should reject with ProductInvalidDate if the selling date is after the current date", async () => {
            await expect(productDAO.sellProduct("model", 5, "2024-07-07"))
                .rejects.toThrow(ProductInvalidDate);
        });
    ...
    describe("getProducts", () => {
        test("should resolve with a list of products for a valid model grouping", async () => {
            ...
            expect(result).toEqual([new Product(100, "model", Category.LAPTOP, "2024-06-06", "details", 10)]);
        ...
        test("should resolve with a list of products for a valid category grouping", async () => {
            ...
            expect(result).toEqual([
                ...
                new Product(150, "model2", Category.LAPTOP, "2024-06-07", "details2", 5),
    ...
    describe("getAvailableProducts", () => {
        test("should resolve with a list of available products for a valid model grouping", async () => {
            const mockDBGet = jest.spyOn(db, "get").mockImplementation((sql, params, callback) => {
                callback(null, {
                    sellingPrice: 100,
                    model: "model",
            ...
            const result = await productDAO.getAvailableProducts("model", null, "model");
            expect(result).toEqual([new Product(100, "model", Category.LAPTOP, "2024-06-06", "details", 10)]);
        ...
        test("should resolve with a list of available products for a valid category grouping", async () => {
            const mockDBAll = jest.spyOn(db, "all").mockImplementation((sql, params, callback) => {
                callback(null, [
                    {
                        sellingPrice: 100,
        ...
        test("should resolve with a list of all available products for no grouping", async () => {
            const mockDBAll = jest.spyOn(db, "all").mockImplementation((sql, params, callback) => {
                callback(null, [
                    {
                        sellingPrice: 100,
